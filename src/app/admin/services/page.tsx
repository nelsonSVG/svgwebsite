        ...
        <table className="w-full text-left font-poppins">
          <thead>
            <tr className="border-b border-zinc-800 bg-zinc-900/50">
              <th className="px-6 py-4 text-sm font-medium text-zinc-400">Service</th>
              <th className="px-6 py-4 text-sm font-medium text-zinc-400">Description</th>
              <th className="px-6 py-4 text-sm font-medium text-zinc-400">Icon</th>
          ...
          <tbody className="divide-y divide-zinc-800">
            {loading ? (
              <tr>
                <td colSpan={4} className="px-6 py-10 text-center text-zinc-500 font-poppins">
                  Loading services...
                </td>
              </tr>
            ) : services.length === 0 ? (
              ...
              services.map((service) => (
                <tr key={service.id} className="hover:bg-zinc-900/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-medium text-white font-poppins">{service.title}</div>
                  </td>
                  <td className="px-6 py-4 text-zinc-400 max-w-xs truncate font-poppins">
                    {service.description}
                  </td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 bg-zinc-800 rounded text-xs flex items-center gap-2 w-fit font-poppins">
                      <Layout size={14} />
                      {service.icon_name}
                    </span>
                  </td>
